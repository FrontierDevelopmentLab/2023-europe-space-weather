...
class NeRFDataModule(LightningDataModule):
    def __init__(self, hparams):
        ...
        valid_rays, valid_times, valid_images = self._flatten_data(
            [v for i, v in enumerate(rays) if i == test_idx],
        ...
        valid_rays, valid_times, valid_images = (
            np.array_split(valid_rays, n_batches),
            np.array_split(valid_times, n_batches),
            np.array_split(valid_images, n_batches),
        )
        self.valid_rays, self.valid_times, self.valid_images = (
            valid_rays,
            valid_times,
            valid_images,
        )
        self.test_kwargs = {
            "focal": focal_lengths[test_idx],
            "resolution": images[test_idx].shape[0],
        }

        # load all training rays
        rays, times, images = self._flatten_data(
            [v for i, v in enumerate(rays) if i != test_idx],
            [v for i, v in enumerate(times) if i != test_idx],
            [v for i, v in enumerate(images) if i != test_idx],
        )
        ...
        rays, times, images = (
            rays[not_nan_pixels],
            times[not_nan_pixels],
            images[not_nan_pixels],
        )
        ...
        rays, times, images = (
            np.array(np.split(rays, n_batches), dtype=np.float32),
        ...
        # to dataset
        self.train_rays, self.train_times, self.train_images = (
            batch_file_rays,
            batch_file_times,
            batch_file_images,
        )
        ...
        self.valid_data = BatchesDataset(
            self.valid_rays, self.valid_times, self.valid_images
        )
        self.train_data = NumpyFilesDataset(
            self.train_rays, self.train_times, self.train_images
        )
        self.points_data = RandomCoordinateDataset(
            [21, 200], [np.min(times), np.max(times)], self.points_batch_size
        )

    def _flatten_data(self, rays, times, images):
        flat_rays = np.concatenate(rays)
        flat_images = np.concatenate(
            [i.reshape((-1, images[0].shape[-1])) for i in images]
        )
        flat_time = np.concatenate(
            [
                np.ones(i.shape[:2], dtype=np.float32).reshape((-1, 1)) * t
    ...
    def train_dataloader(self):
        ...
        points_loader = DataLoader(
            ...
            sampler=RandomSampler(
                self.train_data, replacement=True, num_samples=len(self.train_data)
            ),
        )
...
class NumpyFilesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return [
            np.copy(np.load(batch_file, mmap_mode="r")[idx])
            for batch_file in self.paths
        ]
...
def get_data(config_data):
    ...
    if len(s_maps_pB) == 0 or len(s_maps_tB) == 0:
        raise ValueError("No data found in {} or {}".format(data_path_pB, data_path_tB))
    elif len(s_maps_pB) != len(s_maps_tB):
        raise ValueError(
            "Different number of data found in {} and {}".format(
                data_path_pB, data_path_tB
            )
        )
    ...
    with multiprocessing.Pool(os.cpu_count()) as p:
        data_pB = [
            d for d in tqdm(p.imap(_load_map_data, s_maps_pB), total=len(s_maps_pB))
        ]
        data_tB = [
            d for d in tqdm(p.imap(_load_map_data, s_maps_tB), total=len(s_maps_tB))
        ]
    ...
    ref_wavelength = (
        Map(s_maps_tB[0]).wavelength.value
        if Map(s_maps_tB[0]).wavelength is not None
        else 0
    )
...
def _load_map_data(map_path):
    ...
    focal = (0.5 * W) / np.arctan(
        0.5 * (scale * W * u.pix).to(u.deg).value * np.pi / 180
    )
    ...
    pose = (
        pose_spherical(
            -s_map.heliographic_longitude.to(u.deg).value,
            s_map.heliographic_latitude.to(u.deg).value,
            s_map.dsun.to(u.solRad).value,
        )
        .float()
        .numpy()
    )
    ...
    all_rays = np.stack(
        get_rays(image.shape[0], image.shape[1], s_map.reference_pixel, focal, pose), -2
    )
...
class RandomCoordinateDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        r = (
            self.radius[0]
            + (self.radius[1] - self.radius[0]) * random_coords[:, 0] ** 2
        )  # sample more points closer to inner boundary
