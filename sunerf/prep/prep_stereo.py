...
if __name__ == '__main__':
    ...
    p.add_argument('--stereo_file_path', type=str,
                   default='/mnt/nerf-data/stereo_2012_08_converted_fov/195/*.fits',
                   help='search path for ITI converted maps of STEREO.')
    ...
    # Load paths
    stereo_paths = sorted(glob(args.stereo_file_path))

    # Load maps
    with multiprocessing.Pool(os.cpu_count()) as p:
        p.starmap(_loadMLprepMap,
                  zip(stereo_paths, repeat(args.output_path), repeat(args.scale),
