...
for i, timei in tqdm(enumerate(pd.date_range(loader.start_time, loader.end_time, n_points)), total=n_points):
    densities_in_cube = []
    velocities_in_cube = []
    for j, theta in tqdm(enumerate(th)):
        ...
        densities_in_cube.append(density)
        velocities_in_cube.append(velocity)
        ...
        plt.axis('on')
        fig.savefig(os.path.join(video_path_dens, f'dens_cube_slice_{j:03d}_{i:03d}.jpg'), dpi=100)
        plt.close(fig)
        ...
        plt.axis('on')
        fig.savefig(os.path.join(video_path_dens, f'velocity_cube_slice_{j:03d}_{i:03d}.jpg'), dpi=100)
        plt.close(fig)
    densities.append(densities_in_cube)
    velocities.append(velocities_in_cube)
