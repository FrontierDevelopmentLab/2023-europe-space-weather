...
# Input to the model
x = torch.randn(1, 3, 2048, 2048, requires_grad=True)
...
torch.onnx.export(net,                       # model being run
                  ...
                  "onboard_net_2048.onnx",        # where to save the model (can be a file or file-like object)
...
torch.onnx.export(compressor,                       # model being run
                  x,                         # model input (or a tuple for multiple inputs)
                  "onboard_compressor_y_2048.onnx",        # where to save the model (can be a file or file-like object)
                  export_params=True,        # store the trained parameter weights inside the model file
                  opset_version=11,          # the ONNX version to export the model to
                  do_constant_folding=True,  # whether to execute constant folding for optimization
                  input_names = ['input'],   # the model's input names
