...
if __name__ == "__main__":
    ...
    for batch in timeseries_batches:
        ...
        resulting_images_for_batch_cor1 = Fido.search(time_requested_for_batch, a.Instrument("SECCHI"), a.Detector("COR1"))
        resulting_images_for_batch_cor2 = Fido.search(time_requested_for_batch, a.Instrument("SECCHI"), a.Detector("COR2"))
    ...
    try:
        os.makedirs(cor1_folder, exists_ok = True, mode = 0o777)
    except OSError as e:
        logging.error("Error when creating Cor1 Folder = {} - {}".format(cor1_folder, e))

    try:
        os.makedirs(cor2_folder, exists_ok = True, mode = 0o777)
    except OSError as e:
        logging.error("Error when creating Cor2 Folder = {} - {}".format(cor2_folder, e))
    
    # Download data
    for cor1_batch in cor1_batches:
        Fido.fetch(cor1_batch, "{}/{file}".format(cor1_folder))
    for cor2_batch in cor2_batches:
        Fido.fetch(cor2_batch, "{}/{file}".format(cor2_folder))
